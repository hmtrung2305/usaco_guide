...
int main() {
	...
	std::cin >> numTestcase;
	while (numTestcase--) {
		Line<double> line1, line2, segmentIntersect;
		std::cin >> line1 >> line2;
		Point<double> pointIntersect;
		if (intersectClosedSegments(line1, line2, pointIntersect)) {
			std::cout << std::fixed << std::setprecision(2) << pointIntersect << '\n';
		} else if (intersectSegments(line1, line2, segmentIntersect)) {
			if (segmentIntersect.pointA == segmentIntersect.pointB()) {
				std::cout << std::fixed << std::setprecision(2) << segmentIntersect.pointA << '\n';
			} else {
				Point<double> pointA = segmentIntersect.pointA;
				Point<double> pointB = segmentIntersect.pointB();
				if (pointA.x > pointB.x) {
					swap(pointA, pointB);
				} else if (pointA.x == pointB.x && pointA.y > pointB.y) {
					swap(pointA, pointB);
				}
				std::cout << std::fixed << std::setprecision(2) << pointA << ' ' << pointB << '\n';
			}
		} else {
			std::cout << "none" << '\n';
		}
