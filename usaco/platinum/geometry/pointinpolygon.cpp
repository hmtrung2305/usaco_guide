...
int main() {
	...
	while (std::cin >> numPointPolygon && numPointPolygon != 0) {
		std::vector<Point<double>> pointPolygon(numPointPolygon);
		for (Point<double> &point : pointPolygon) {
			std::cin >> point;
		}
		int numQuery;
		std::cin >> numQuery;
		while (numQuery--) {
			Point<double> pointQuery;
			std::cin >> pointQuery;
			Point<double> ray = Point<double>(2e9, pointQuery.y + 1);
			int countIntersect = 0;
			Point<double> pointIntersect;
			for (int index = 0; index < numPointPolygon; index++) {
				if (intersectClosedSegments(Line<double>(pointPolygon[index], pointPolygon[(index + 1) % numPointPolygon]), Line<double>(pointQuery, ray), pointIntersect)) {
					countIntersect++;
				}
				if (onSegment(pointQuery, Line<double>(pointPolygon[index], pointPolygon[(index + 1) % numPointPolygon]))) {
					std::cout << "on\n";
					countIntersect = -1;
					break;
				}
			...
			if (countIntersect != -1) {
				std::cout << (countIntersect % 2 == 0 ? "out" : "in") << '\n';
			}
		}
	}
	return 0;
