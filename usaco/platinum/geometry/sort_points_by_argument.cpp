...
/// Distances
template<class TypeDataObject>
inline TypeDataObject sumSquare(const Point<TypeDataObject> &point) {
	return point * point;
}
template<class TypeDataObject>
inline TypeDataObject lengthVector(const Point<TypeDataObject> &point) {
	return sqrt(sumSquare(point));
}
template<class TypeDataObject1, class TypeDataObject2>
...
int main() {
	...
	int numPoint;
	std::cin >> numPoint;
	std::vector<Point<int>> points(numPoint);
	for (Point<int> &point : points) {
		std::cin >> point;
	}
	sort(points.begin(), points.end(), [](const Point<int> &point1, const Point<int> &point2) {
		return angle(point1) < angle(point2);
	});
